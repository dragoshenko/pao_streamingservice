...
public class Main {
    public static void main(String[] args) {
        ...
        List<String[]> records = csvReaderService.read();

        for (String[] record : records) {
            String type = record[0];
            switch (type) {
                case "artist":
                    String artistName = record[1];
                    String numeScena = record[2];
                    int age = Integer.parseInt(record[3]);
                    Artist artist = new Artist(artistName, numeScena, age);
                    System.out.println("Artist: " + artist.getNume() + ", Nume Scena: " + artist.getNumeScena() + ", Age: " + artist.getVarsta());
                    break;
                case "genMuzical":
                    String genreName = record[1];
                    GenMuzical genre = new GenMuzical(genreName);
                    System.out.println("Genre: " + genre.getGen());
                    break;

                case "album":
                    String albumTitle = record[1];
                    String genMuzical = record[2];
                    String artistName2 = record[3];
                    String numeScena2 = record[4];
                    int age2 = Integer.parseInt(record[5]);
                    GenMuzical albumGenre = new GenMuzical(genMuzical);
                    Artist albumArtist = new Artist(artistName2, numeScena2, age2);
                    Album album = new Album(albumTitle, albumGenre, albumArtist);
                    System.out.println("Album: " + album.getNume() + ", Genre: " + album.getGenMuzical().getGen() + ", Artist: " + album.getArtist().getNumeScena());
                    break;


                case "melodie":
                    String songTitle = record[1];
                    int duration = Integer.parseInt(record[2]);
                    int plays = Integer.parseInt(record[3]);
                    String albumTitle2 = record[4];
                    Melodie song = new Melodie(songTitle, duration, plays);
                    System.out.println("Song: " + song.getNume() + ", Duration: " + song.getDurata() + ", Plays: " + song.getNumarStreamuri() + ", Album: " + albumTitle2);
                    break;
                default:
        ...
        auditService.logAction("Added melodie " + melodie1.getNume());
        auditService.logAction("Added melodie " + melodie2.getNume());
        auditService.logAction("Added melodie " + melodie3.getNume());
        ...
        auditService.logAction("Added album " + album.getNume());
        ...
        auditService.logAction("Added melodie " + melodie1_1.getNume());
        auditService.logAction("Added melodie " + melodie2_1.getNume());
        auditService.logAction("Added melodie " + melodie3_1.getNume());
        ...
        auditService.logAction("Added album " + album_1.getNume());
 ...
 */
        ...
        List<String[]> records2 = csvReaderService.read();
